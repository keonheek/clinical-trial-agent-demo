...
| **방문 빈도/기간(trial burden), 재정 부담, 동반자 필요성** | **판정 로직은 아웃, 정보 노출은 인** (아래 실무 검증 참조) | 프로토콜 원문에 이미 명시된 조항(예: "교통비 지원", "동반자 필수 동행")이 있으면 그대로 보여주기만 함 — 판정하지 않음. 없는 정보를 추정/계산하지 않음 |

### 실무 검증: 박서연(CRC 8년차) 페르소나 검토 (2026-08-09)
